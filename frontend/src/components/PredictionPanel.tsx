...
import { AlertTriangle, Clock, MapPin, Wind, Thermometer, CloudRain, ShieldAlert, History, CalendarClock, Newspaper } from 'lucide-react';
    ...
    <div ref={panelRef} className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 rounded-xl border border-black/5 dark:border-white/10 bg-white/60 dark:bg-black/40 backdrop-blur-md panel-item shadow-lg">
          <div className="flex items-center gap-2 text-neutral-500 dark:text-white/60 mb-1">
            <Clock className="w-4 h-4" />
            <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Base Travel Time</span>
          </div>
          <div className="text-2xl font-black text-neutral-900 dark:text-white">{prediction.base_travel_days} Days</div>
        </div>
        <div className="p-4 rounded-xl border border-black/5 dark:border-white/10 bg-white/60 dark:bg-black/40 backdrop-blur-md panel-item shadow-lg">
          <div className="flex items-center gap-2 text-neutral-500 dark:text-white/60 mb-1">
            <CalendarClock className="w-4 h-4" />
            <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Expected Delivery</span>
          </div>
          <div className="text-lg font-black text-neutral-900 dark:text-white">{prediction.expected_delivery_date}</div>
        </div>
      </div>
      ...
      <div className="p-4 rounded-xl border border-black/5 dark:border-white/10 bg-white/60 dark:bg-black/40 backdrop-blur-md panel-item shadow-lg">
        <div className="flex items-center gap-2 text-neutral-500 dark:text-white/60 mb-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em]">News Sentiment</span>
        </div>
        <div className="text-xs font-semibold text-neutral-800 dark:text-white/90 leading-relaxed">
          Score: {prediction.sentiment_score} (Signal: {prediction.disruption_signal})
        </div>
        <div className="text-xs font-medium text-neutral-600 dark:text-white/80 mt-1 leading-relaxed">
          {prediction.sentiment_reason}
        </div>
      </div>
      ...
      <div className="p-4 rounded-xl border border-black/5 dark:border-white/10 bg-white/60 dark:bg-black/40 backdrop-blur-md panel-item shadow-lg">
        <div className="flex items-center gap-2 text-neutral-500 dark:text-white/60 mb-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Supplier & Logistics</span>
        </div>
        <div className="text-xs font-semibold text-neutral-800 dark:text-white/90 leading-relaxed">
          Supplier Risk: {prediction.supplier_risk.risk_category} (Score: {prediction.supplier_risk.composite_risk_score.toFixed(2)})
        </div>
        <div className="text-xs font-medium text-neutral-600 dark:text-white/80 mt-1 leading-relaxed">
          {prediction.supplier_risk_reason}
        </div>
        <div className="text-xs font-semibold text-neutral-800 dark:text-white/90 mt-3 leading-relaxed">
          Historical Disruptions: {prediction.historical_disruptions.toFixed(1)} avg/day
        </div>
        <div className="text-xs font-semibold text-neutral-800 dark:text-white/90 mt-3 leading-relaxed">
          LSTM Probability: {prediction.lstm_probability.toFixed(1)}%
        </div>
        <div className="text-xs font-semibold text-neutral-800 dark:text-white/90 mt-1 leading-relaxed">
          XGBoost Probability: {prediction.xgboost_probability.toFixed(1)}%
        </div>
        <div className="text-xs font-medium text-neutral-600 dark:text-white/80 mt-1 leading-relaxed">
          {prediction.model_probability_reason}
        </div>
        <div className="text-xs font-medium text-neutral-600 dark:text-white/80 mt-3 leading-relaxed">
          {prediction.transport_reason}
        </div>
      </div>
