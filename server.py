...
class PredictionResponse(BaseModel):
    ...
    supplier_risk_reason: str
    transport_reason: str
    lstm_probability: float
    xgboost_probability: float
    model_probability_reason: str
...
def get_top_causes(origin_city, transport_mode, weather_sev, sentiment, supplier_risk_cat,
    ...
    if sentiment <= -0.7:
        causes.append({
            ...
            'detail': f'Recent news from {origin_city} is very negative (score {round(sentiment,2)}), with likely reports of strikes, protests, flooding, or political unrest affecting movement.'
        })
    elif sentiment < -0.4:
        causes.append({
            'text': f'Negative news sentiment in {origin_city}',
            'detail': f'News from {origin_city} leans negative (score {round(sentiment,2)}), with possible reports of labour issues, road blockages, or weather-related movement constraints.'
        })
    if supplier_risk_cat == 'High':
        ...
        causes.append({
            ...
            'detail': f'Moderate composite risk score of {round(composite_risk,2)} suggests occasional past delays and moderate regional exposure; not alarming but worth monitoring.'
        })
    if transport_mode == 'Road':
        ...
        causes.append({'text': 'Air transport selected', 'detail': 'Fastest option for base travel time, but minor delays can still happen due to weather checks, slot congestion, or cargo handling queues.'})
...
def get_sentiment_reason(origin_city, sentiment, disruption_signal):
    if sentiment <= -0.7:
        return (
            f"News sentiment is very negative ({round(sentiment,2)}) with disruption signal {round(disruption_signal,2)}; "
            f"likely reports include strikes, protests, flooding, or local civic disruptions around {origin_city}."
        )
    if sentiment < -0.4:
        return (
            f"News sentiment is negative ({round(sentiment,2)}) with disruption signal {round(disruption_signal,2)}; "
            f"possible local reports include traffic blockades, labor tension, or weather disturbances."
        )
    return (
        f"News sentiment is neutral-to-positive ({round(sentiment,2)}) with disruption signal {round(disruption_signal,2)}; "
        f"no strong media-driven disruption warning currently stands out for {origin_city}."
    )


def get_supplier_reason(risk_category, composite_risk):
    score = round(float(composite_risk), 2)
    if risk_category == "High":
        return f"Supplier risk is High (score: {score}) due to stronger signals of delivery failures, regional strike exposure, and weaker operational resilience."
    if risk_category == "Medium":
        return f"Supplier risk is Medium (score: {score}) due to occasional past delays or moderate regional exposure; not alarming, but worth monitoring."
    return f"Supplier risk is Low (score: {score}) with relatively stable delivery patterns and fewer disruption indicators."


def get_transport_reason(transport_mode):
    if transport_mode == "Road":
        return "Road mode can face congestion, road blockages, local weather impact, and checkpoint delays."
    if transport_mode == "Rail":
        return "Rail mode depends on schedules; terminal queues and timetable shifts can add delay."
    if transport_mode == "Air":
        return "Air mode has low base travel time, but weather checks and airport handling can still cause small delays."
    return "Road+Rail mode includes transfer handoffs; timing mismatch between legs can add delay."


def get_model_probability_reason(lstm_prob, xgb_prob, predicted_delay):
    return (
        f"LSTM shows {round(lstm_prob*100,1)}% disruption chance from past delay pattern, "
        f"and XGBoost gives final risk at {round(xgb_prob*100,1)}% after combining weather, route, supplier, and sentiment signals. "
        f"At this level, expected delay is about {predicted_delay} day{'s' if predicted_delay != 1 else ''}."
    )
...
def load_assets():
    ...
    except Exception as e: print(f"[WARN] XGBoost fail: {e}")
    ...
    except Exception as e: print(f"[WARN] IsoForest fail: {e}")
    ...
    except Exception as e: print(f"[WARN] Encoders fail: {e}")

    try:
        print("Loading Feature List...")
        with open(MODELS_DIR + 'feature_list.json') as f:
            ASSETS['xgb_features'] = json.load(f)
    except Exception as e: print(f"[WARN] Feature list fail: {e}")
    ...
    except Exception as e: print(f"[WARN] Datasets fail: {e}")
...
def predict(req: PredictionRequest):
    ...
    try:
        ...
        return PredictionResponse(
            ...
            disruption_signal=round(disruption_signal, 2),
            sentiment_reason=get_sentiment_reason(req.origin_city, sentiment, disruption_signal),
            supplier_risk_reason=get_supplier_reason(risk['risk_category'], risk['composite_risk_score']),
            transport_reason=get_transport_reason(req.transport_mode),
            lstm_probability=round(lstm_prob * 100, 1),
            xgboost_probability=round(xgb_prob * 100, 1),
            model_probability_reason=get_model_probability_reason(lstm_prob, xgb_prob, predicted_delay)
        )
