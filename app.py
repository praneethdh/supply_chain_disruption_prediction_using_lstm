...
def render_predictor(assets, sentiment_lookup, d4, merged, city_hist_avg, city_rolling):
    ...
    if st.button("Predict Disruption Risk", type="primary"):
        ...
        o_lat, o_lon = CITY_COORDS[origin_city]
        d_lat, d_lon = CITY_COORDS[destination_city]
        ...
        arc_layer = pdk.Layer(
            ...
            data=[{
                "source": [o_lon, o_lat],
                "target": [d_lon, d_lat],
                "source_color": [248, 113, 113, 200],
                "target_color": [74, 222, 128, 200],
            }],
        ...
        scatter_layer = pdk.Layer(
            "ScatterplotLayer",
            data=[
                {"name": origin_city, "coordinates": [o_lon, o_lat], "color": [248, 113, 113, 200]},
                {"name": destination_city, "coordinates": [d_lon, d_lat], "color": [74, 222, 128, 200]}
            ],
        ...
        with det_col1:
            ...
            if sentiment == 0.0 and disruption_signal == 0.0:
                st.write(f"- **News Sentiment:** 0.00 — ⚠️ No data available")
                st.caption(
                    f"No news sentiment data is available for {origin_city} in this month. "
                    f"The model uses a neutral default (0) which does not influence the risk score. "
                    f"Only 10 of 15 cities have sentiment data in the dataset."
                )
            else:
